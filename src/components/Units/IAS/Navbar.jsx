...
const Navbar = () => {
  ...
  return (
    <header className="fixed bg-[#00000080] md:h-[4.8rem] inset-x-0 top-0 z-50">
        <nav className="flex items-center justify-between px-6 lg:px-8 md:py-0 py-4" aria-label="Global">
          <div className="flex lg:flex-1">
            <Link to="" onClick={() => window.location.href='/IAS'} className="-m-1.5 p-1.5">
              <img
                src={IAS}
                alt="ias"
                className='md:w-[120px] h-[70px] w-[130px]'
              />
            </Link>
          </div>
          ...
          {pcnavigation.map((item) => (
              <a key={item.name} href={item.href} className="text-base font-semibold leading-6 hover:text-[#078107] text-white">
                {item.name}
              </a>
            ))}
          </div>

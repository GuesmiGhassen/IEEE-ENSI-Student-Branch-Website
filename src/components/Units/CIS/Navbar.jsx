    ...
    <header className="fixed bg-[#00000080] h-[4.5rem] inset-x-0 top-0 z-50">
        <nav className="flex items-center justify-between px-6 lg:px-8 md:py-0 py-4" aria-label="Global">
          <div className="flex lg:flex-1">
            <Link to="" onClick={() => window.location.href='/CIS'} className="-m-1.5 p-1.5">
              <img
                src={CIS}
                alt="cis"
          ...
          {pcnavigation.map((item) => (
              <a key={item.name} href={item.href} className="text-base font-semibold leading-6 hover:text-[#4A8BD2] text-white">
                {item.name}
              </a>
            ))}
          </div>
